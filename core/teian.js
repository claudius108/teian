...
window.teian = {
	...
	"annotator" : [
		function(oAnnotator, sAnnotatorType) {
			var utils = teian.utils;
			teian.utils.restoreSelection();
			var oSelection = rangy.getSelection()
			, sOperationType = utils.sOperationType;
			;
			if (oSelection == "" && sAnnotatorType != 'insert') {
				alert(teian._errors[0]);
				return;
			}
			var sParentName = (oSelection.anchorNode.nodeName == '#text') ? oSelection.anchorNode.parentNode.nodeName : oSelection.anchorNode.nodeName
			;
	...
	"editEntity" : function() {
		var utils = teian.utils
		, oEntityToClear = utils.oEntityToClear
		, sEntityToClearName = oEntityToClear.nodeName
		;
		...
		var oRange = rangy.createRange()
		, sHTMLAnnotatorID = $x.xpath("simpath:instance('annotators')//teian:annotator[@name = '" + sEntityToClearName + "']/@id")[0].value
		;
...
$(document).ready(function() {
	...
	, sModuleBaseURI = sDocumentURL.substring(0, sDocumentURL.indexOf("core/teian.html"))
	, utils = teian.utils
	, _errors = teian._errors
	;
	utils.baseURI = sModuleBaseURI;
	//initialize the error messages
	$x.instance('ui-lang').load($x.parseFromString("<teian:lang xmlns:teian=\"http://kuberam.ro/ns/teian\" xmlns=\"http://www.w3.org/1999/xhtml\"/>"));
	$x.submission({
	...
	$x.instance('annotators').load($x.parseFromString("<teian:annotators xmlns:teian=\"http://kuberam.ro/ns/teian\"/>"));
	$x.submission({
	...
	//themes roller
	$.themes.setDefaults({ 
		"defaultTheme" : "lefrog" // The ID of the default theme, first one if blank 
	});
	$.themes.init(
		{
	...
	//get the tei-ann module's base uri
	var sAnnotatorIDs = ""
	, sEditableAnnotatorIDs = ""
	, oDataRoot = $("#teian-content > *")[0]
	, sDataRootPrefix = oDataRoot.prefix ? oDataRoot.prefix + ":" : "" 
	;
	...
	$($x.xpath("simpath:instance('annotators')//teian:annotator[@editable = 'true']/@name")).each(function(index) {
	...
	, oContextMenuItems = $("ul[appearance='kyer-context-menu']").find("a[id]")
	, oMenuItems = oToolbarMenuItems.add(oContextMenuItems)
	;
	oMenuItems.each(function(index) {
		var oHTMLAnnotator = $(this)
		, oHTMLAnnotator0 = this
		, sAnnotatorId = oHTMLAnnotator.attr('id')
		, sHTMLAnnotatorType = oHTMLAnnotator.attr('appearance')
		;
		if (sAnnotatorIDs.indexOf(sAnnotatorId) != -1 && sAnnotatorId != '') {
			var oAnnotator0 = $x.xpath("simpath:instance('annotators')//teian:annotator[@id = '" + sAnnotatorId + "']")[0]
			, oAnnotator = $(oAnnotator0)
			, oLang0 = $x.xpath("simpath:instance('ui-lang')//teian:annotator[@id = '" + sAnnotatorId + "']")[0]
			, oLang = $(oLang0)
			, sAnnotatorType = oAnnotator.attr('type-code')
			;
			...
			oHTMLAnnotator.html((sHTMLAnnotatorType == 'image-button' ? "<img src=\"" + sModuleBaseURI + "config/images/" + $x.xpath("/teian:annotator/teian:annotator-icon-name/text()", oAnnotator0) + "\"/>": "") + $x.serializeToString($x.xpath("/teian:annotator/teian:toolbar-button-label", oLang0)[0]));
			//set the possible parents
			oHTMLAnnotator0['sPossibleParents'] = $x.xpath("normalize-space(/teian:annotator/teian:annotator-possible-parent-element-names/text())", oAnnotator0);

			//generate the annotation markup
			if (sAnnotatorType != 'action') {
				var oAnnotatorMarkup = $(typeof(document.createElementNS) == 'undefined' ? document.createElement(sDataRootPrefix + oAnnotator.attr('name')) : document.createElementNS(oDataRoot.namespaceURI, sDataRootPrefix + oAnnotator.attr('name')))
				;
			...
			switch(sAnnotatorType) {
				...
				case 'selected-wrap-server':
					oHTMLAnnotator.click(function() {
						...
						teian.ui['selected-wrap-server-annotator'].dialog('option', 'title', $x.xpath("simpath:instance('ui-lang')//teian:annotator[@id = '" + sAnnotatorId + "']/teian:annotator-panel/teian:annotator-panel-title/text()"));
						teian.ui['selected-wrap-server-annotator'].dialog('open');
					});
					//registering the annotator's main attribute, which will hold the
					//search result - to be removed in v. 2.1
	...
	//load the data model containing the 'selected-wrap-server'annotators' UI
	//this have to be done separately as the UI is common to all annotators
	if (teian.ui['selected-wrap-server-annotators-counter']) {
		$($x.xpath("simpath:instance('annotators')/teian:annotators/teian:common-panels/teian:common-panel[@id = 'selected-wrap-server-annotator-panel']")[0]).appendTo("body");
	}
	//initialize rangy
	utils.oSavedSelection = null;
...
$(window).load(function() { $("#kFloadingMask").fadeOut(); });